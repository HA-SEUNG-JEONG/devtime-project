...
const TermsAgreement = () => {
  ...
  const { onChange } = register('termsAgreed', {
    required: '이용약관에 동의해 주세요',
    validate: value => value === true || '이용약관에 동의해 주세요',
  });

  const handleChange = (checked: boolean) => {
    setValue('termsAgreed', checked, { shouldValidate: true });
    onChange({ target: { value: checked, name: 'termsAgreed' } });
  };
  ...
  return (
    <div className="flex flex-col items-start gap-2 w-[420px]">
      <div className="flex flex-row justify-between items-center gap-2 w-full h-[18px]">
        <span className="text-14m text-gray-600 flex items-center">
          이용약관
        </span>
        <div className="flex flex-row items-center gap-1">
          <span className="text-14m text-primary-30 flex items-center">
            동의함
          </span>
          <Checkbox
            usage="regular"
