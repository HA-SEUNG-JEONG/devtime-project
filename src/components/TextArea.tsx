...
const TextArea: React.FC<TextAreaProps> = ({
  ...
  const baseContainerClasses = `flex flex-row items-start px-4 gap-[10px] w-[568px] h-[84px] bg-[#F9FAFB] rounded-[5px] ${className}`;

  // Ready 상태 - Placeholder만 표시
  if (variant === 'ready') {
    return (
      <div className={`${baseContainerClasses} py-3`} {...props}>
        <span className="text-16m text-[#CCD0D6] flex items-center w-[536px] h-5 flex-none grow">
          {placeholder}
        </span>
      </div>
    );
  ...
  return (
    <div className={`${baseContainerClasses} pt-3 pb-0`} {...props}>
      <span className="text-16m text-[#4B5563] block w-[536px] h-[80px] overflow-y-auto wrap-break-word whitespace-normal">
        {value ||
